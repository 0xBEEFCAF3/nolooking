...
pub async fn get_recommended_channels() -> Result<Recommendations, hyper::http::Error> {
    ...
    // Recommend 6 high liquidity channels
    // Recommend 4 smaller edge nodes
...
async fn get_node(pubkey: &str) -> Result<Node, hyper::http::Error> {
    ...
    let mut node: Node = serde_json::from_str(&body_str).unwrap();
    // if node.sockets.contains(",") {
    //     node.sockets = node.sockets.split(",").;
    // }
    Ok(node)
}
